...
void traverse_tree_modified(struct tree_node *ptr,uint sorted [],int *counter,
			    struct stack_node **stacks);
...
void find_all_pairs_modified(struct tree_node *ptr,uchar *T,ulong N,ulong k,ulong startpos[],
			     uint sorted[],int output);
...
void distribute_k_1(int threads,uchar *T,ulong startpos[],uint sorted[],int **A,struct  tree_node* ptr,
		    ulong k,ulong N,int output,int min, bool index[]);
